...
+
For this reason, create a service account, the appropriate cluster role binding, and hostPath access by issuing:
+
....
kubectl apply -f appsody-service-account.yaml
kubectl apply -f appsody-cluster-role-binding.yaml
oc adm policy add-scc-to-user hostaccess -z appsody-sa -n kabanero
....
+
